...
impl Cpu {
        ...
        match self.cpu_state {
            CpuState::Fetch => {
                ...
                if self.current_opcode == PREFIX_OPCODE {
                    ...
                    if OPCODE_MACHINE_CYCLES[self.current_opcode as usize] == 1 {
                        match self.exexute(self.current_opcode, memory, 1, &mut 0) {
            ...
            CpuState::FetchPrefix => {
                ...
                if PREFIX_OPCODE_MACHINE_CYCLES[self.current_opcode as usize] == 2 { //2 b/c you always have to fetch the prefix
                    match self.exexute_prefix(self.current_opcode, memory, 1, &mut 0) {
                        ExecuteStatus::Completed => self.cpu_state = CpuState::InterruptHandle,
                        ExecuteStatus::Running => (),
                        ExecuteStatus::Error => panic!("Error Executing opcode"),
                    }
            ...
            CpuState::Execute { mut machine_cycle, mut temp_reg } => {
                machine_cycle += 1;
                match self.exexute(self.current_opcode, memory, machine_cycle, &mut temp_reg) {
                    ExecuteStatus::Completed => self.cpu_state = CpuState::InterruptHandle,
                    ExecuteStatus::Running => (),
                    ExecuteStatus::Error => panic!("Error Executing opcode"),
                }    
    ...
    */
    pub fn exexute(&mut self, opcode: u8, memory: &mut Memory, machine_cycle: u8, temp_reg: &mut u16) -> ExecuteStatus {
        match opcode {
            ...
            0x04 => Cpu::inc_r8(&mut self.b, &mut self.f, machine_cycle),                                            //INC_B
     ...
     * INSTRUCTION LENGTH: 1
     */
    fn inc_r8(reg: &mut u8, flag_reg: &mut u8, machine_cycle: u8) -> ExecuteStatus {
        match machine_cycle {
            1 => {
                *reg += 1;
    ...
    */
    pub fn exexute_prefix(&mut self, opcode: u8, memory: &mut Memory, machine_cycle: u8, temp_reg: &mut u16) -> ExecuteStatus {
        match opcode {
